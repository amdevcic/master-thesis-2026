...
predefined paths while AMRs plan their path for every new mission. AGV-s often
have a controller system which commands and controls all robots in the network.
In some cases AMR-s might be the solution that requires less human intervention
and input and may be more efficient, however AGV-s allow for more control and
predictability. The line separating these two approaches is becoming
increasingly blurred as some AGVs can now re-plan their paths around obstacles.
This thesis is focused on pre-planned paths and will refer to the vehicles
simply as mobile robots.
